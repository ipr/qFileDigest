...
void CImploderExploder::DeplodeImploder()
{
	if (m_in_list.size < 0x30) 
	{
		...
		throw IOException("too short to be a FImp file");
	}
	...
	{
		throw IOException("compressed size not even");
	}
	if (m_HeaderMetadata.m_end_off < 14) 
	{
		throw IOException("compressed size too short");
	}
	if ((m_HeaderMetadata.m_end_off + 0x2E) > m_in_list.size) 
	{
		throw IOException("not enough input data");
	}
	if ((m_HeaderMetadata.m_end_off + 0x26) > m_HeaderMetadata.m_out_len) 
	{
		throw IOException("output length less than input");
	}
	...
	{
		throw IOException("can't allocate memory");
	}
	...
	if (ok == false)
	{
		throw IOException("can't deplode");
	}
}
 
void CImploderExploder::Save(const char *szOutputFile) 
{
...
void CImploderExploder::Load(const char *szInputFile)
{
	...
	// no longer necessary
	// (closed on leaving scope anyway)
	InputFile.Close();
}

void CImploderExploder::CheckExisting(const char *szOutputFile)
{
